...
assert(dataJson.items && dataJson.items.length === 100, 'Catalogue retains 100 unique IDs and ten Chapter 9 questions');
...
// Check questions 91–100 match baseline
const baselineJson = JSON.parse(cp.execSync('git show a87ed5f:data/hoidap_data.json', { cwd: repoRoot, maxBuffer: 10 * 1024 * 1024 }));
let postMatch = true;
...
assert(postMatch, 'Questions 91–100 match the release baseline');

// 3. Check each Chapter 9 question (81 to 90)
for (let i = 80; i < 90; i++) {
...
assert(
  uiContent.includes('Trường Sinh Tứ Cục: phân định Kim Mộc Thủy Hỏa tứ đại cục'),
  'Chapter 9 hero description explains the chapter curriculum accurately'
);

console.log(`\nChapter 9 release checks: ${passed} passed, ${failed} failed.`);
if (failed > 0) {
  process.exit(1);
}
