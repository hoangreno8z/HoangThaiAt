...
assert(dataJson.items && dataJson.items.length === 100, 'Catalogue retains 100 unique IDs and ten Chapter 7 questions');
...
// Check questions 71–100 match baseline
...
assert(postMatch, 'Questions 71–100 match the release baseline');

// 3. Check each Chapter 7 question (61 to 70)
for (let i = 60; i < 70; i++) {
...
assert(
  uiContent.includes('Chân Hư Tương Phối: phân tích sự phối hợp giữa Chân Thủy dòng sông thật'),
  'Chapter 7 hero description explains the chapter curriculum accurately'
);

console.log(`\nChapter 7 release checks: ${passed} passed, ${failed} failed.`);
if (failed > 0) {
  process.exit(1);
} else {
  process.exit(0);
