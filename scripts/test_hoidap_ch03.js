...
// Chapter 3 release guard: validate the revised content and preserve
// Chapter 1, Chapter 2, and all 70 entries outside this release's scope.
...
const chapterThree = items.filter(item => item.chapter === 3);

if (!rendererOnly) test('Catalogue retains 100 unique IDs and ten Chapter 3 questions', () => {
  assert.equal(items.length, 100);
  assert.deepEqual(items.map(item => item.index).sort((a, b) => a - b),
    Array.from({ length: 100 }, (_, index) => index + 1));
  assert.deepEqual(chapterThree.map(item => item.index).sort((a, b) => a - b),
...
if (!rendererOnly) for (const item of chapterThree) {
...
const fixtureSource = {
  ...
  quote: '水對三叉細認蹤。', note: 'Chỉ chứng minh câu trích.'
};
const fixture = {
  ...baseline.HOIDAP_DATA[20], hanzi: fixtureSource.quote,
...
test('Every Chapter 3 question renders successfully in Reviewed body', () => {
  const { ui } = loadRenderer(rendererCode);
  for (const item of chapterThree) {
...
test('Chapter 3 hero description explains the chapter curriculum accurately', () => {
  const { ui, container } = loadRenderer(rendererCode);
  ui.selectChapter(3);
  assert.ok(container.innerHTML.includes('Kỳ Lộ Giao Xoa: phân tích các giao lộ chữ H, chữ Y, chữ T'));
});

console.log(`\nChapter 3 release checks: ${passed} passed, ${failed} failed.`);
