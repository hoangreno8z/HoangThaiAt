...
// Chapter 2 release guard: validate the revised content and preserve
// Chapter 1 and all 80 entries outside this release's scope.
...
const ch01Sha = 'e6851fe8fbfa18f97bb3dd49980d90d7915ec551';
...
const ch01Data = loadData(ch01Code, `${ch01Sha}:js/hoidap_data.js`);

const items = current.HOIDAP_DATA;
const chapterTwo = items.filter(item => item.chapter === 2);

if (!rendererOnly) test('Catalogue retains 100 unique IDs and ten Chapter 2 questions', () => {
  assert.equal(items.length, 100);
  assert.deepEqual(items.map(item => item.index).sort((a, b) => a - b),
    Array.from({ length: 100 }, (_, index) => index + 1));
  assert.deepEqual(chapterTwo.map(item => item.index).sort((a, b) => a - b),
...
if (!rendererOnly) for (const item of chapterTwo) {
...
const fixtureSource = {
  ...
  quote: '九曲池', note: 'Chỉ chứng minh câu trích.'
};
const fixture = {
  ...baseline.HOIDAP_DATA[10], hanzi: fixtureSource.quote,
...
test('Every Chapter 2 question renders successfully in Reviewed body', () => {
  const { ui } = loadRenderer(rendererCode);
  for (const item of chapterTwo) {
...
test('Chapter 2 hero description explains the chapter curriculum accurately', () => {
  const { ui, container } = loadRenderer(rendererCode);
  ui.selectChapter(2);
  assert.ok(container.innerHTML.includes('Khúc Thủy Chiết Xung: phân tích các địa cuộc đường uốn cong cửu khúc'));
});

console.log(`\nChapter 2 release checks: ${passed} passed, ${failed} failed.`);
