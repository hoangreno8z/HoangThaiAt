...
assert(dataJson.items && dataJson.items.length === 100, 'Catalogue retains 100 unique IDs and ten Chapter 6 questions');
...
// Check questions 61–100 match baseline
...
assert(postMatch, 'Questions 61–100 match the release baseline');

// 3. Check each Chapter 6 question (51 to 60)
for (let i = 50; i < 60; i++) {
...
assert(
  uiContent.includes('Hạp Phong Tiêm Giác: giải mã luồng gió kẹp qua khe cao ốc'),
  'Chapter 6 hero description explains the chapter curriculum accurately'
);

console.log(`\nChapter 6 release checks: ${passed} passed, ${failed} failed.`);
if (failed > 0) {
  process.exit(1);
} else {
  process.exit(0);
