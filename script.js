...
function render(year, month, day, hour) {
    ...
    THAP_LUC_THAN.forEach(than => {
        ...
        stars.forEach(s => {
            ...
            else if (["chu-tuong", "khach-tuong", "tham-chu", "tham-khach", "dinh-xuong"].includes(s.class)) generalHtml += tag;
            else if (["quan-co", "than-co", "dan-co", "ngu-phuc"].includes(s.class)) baseHtml += tag;
            else auxHtml += tag;
        });
        ...
        cell.innerHTML = `
            ...
            <div class="cell-line-5">${baseHtml || '<span class="text-muted">Cơ/Phúc: —</span>'}</div>
            <div class="cell-line-6">${auxHtml || `<span class="text-muted">Cửa: ${data.batMon}</span>`}</div>
        `;
    });
}
